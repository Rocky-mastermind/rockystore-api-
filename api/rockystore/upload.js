...
<html><head><title>Rocky Store Upload</title>
<style>
...
<input id="rawUrl" placeholder="GitHub Raw URL" />
...
async function upload(){
  ...
  try{
    const r=await fetch('/rockystore/upload',{method:'POST',headers:{'Content-Type':'application/json'},
  ...
  let rawCode = "";
  try {
    const r = await axios.get(rawUrl, { timeout: 10000, responseType: "text", transformResponse: [(d) => d] });
    rawCode = String(r.data || "");
  } catch (err) {
    return res.status(400).json({ error: "rawUrl fetch failed: " + (err.response?.status ? `HTTP ${err.response.status}` : err.message) });
  }
  ...
  if (duplicate) return res.status(409).json({ error: `Already exists as ID ${duplicate.id}.` });

  const newEntry = {
    id: db.nextId++, name, version,
    author: resolvedAuthor, category: resolvedCategory, description,
    type: `goat-${kind}`, rawUrl,
    likes: 0, views: 0, installs: 0, likedBy: [],
  ...
  return res.status(200).json({ success: true, id: newEntry.id, name: newEntry.name, type: newEntry.type, author: newEntry.author, version: newEntry.version, category: newEntry.category, owner: newEntry.owner });
};
